...
DB_PATH = os.path.join(BASE_DIR, "anon_bot.db")
...
def update_user(tg_id, **fields):
    ...
    db.execute(f"UPDATE users SET {keys} WHERE tg_id=?", values)
    db.commit()
...
def ensure_user(tg_id):
    ...
    max_code = db.execute("SELECT COALESCE(MAX(user_code), 999) FROM users").fetchone()[0]
    code = max_code + 1

    db.execute(
        "INSERT INTO users (tg_id, state, user_code) VALUES (?, 'new', ?)",
        (tg_id, code)
    )
    db.commit()
...
def anon_code():
    while True:
        ...
        exists = db.execute("SELECT 1 FROM users WHERE anon_code=?", (code,)).fetchone()
        if not exists:
            return code
...
def blocked(a, b):
    return db.execute(
        ...
        WHERE (reporter_id=? AND reported_id=?)
           OR (reporter_id=? AND reported_id=?)
        """,
        (a, b, b, a)
...
def profile(user):
    gender = {"male": "پسر", "female": "دختر"}.get(user.get("gender"), "-")
    return (
...
SKIP_MENU = ReplyKeyboardMarkup([["⏭ رد کردن"]], resize_keyboard=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if param.startswith("anon_"):
            code = param[5:]
            row = db.execute("SELECT * FROM users WHERE anon_code=?", (code,)).fetchone()
            if not row:
                await update.message.reply_text("❌ این لینک معتبر نیست.")
                return

            cols = [x[0] for x in db.execute("SELECT * FROM users LIMIT 1").description]
            owner = dict(zip(cols, row))

            if owner["tg_id"] == tg_id:
                await update.message.reply_text("❌ نمی‌تونی برای خودت پیام ناشناس بفرستی.")
                return
        ...
        if param.startswith("ref_"):
            ...
            if referrer and referrer != tg_id and not user.get("referred_by"):
                update_user(tg_id, referred_by=referrer)
                try:
    ...
    # اگر منتظر بود، از حالت انتظار خارجش کن
    if user.get("state") == "waiting":
        update_user(tg_id, state="idle", waiting_for=None)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "ask_nickname":
        ...
        if not 2 <= len(name) <= 20:
            await message.reply_text("اسم باید بین ۲ تا ۲۰ حرف باشه.")
            return
        update_user(tg_id, nickname=name, state="ask_age")
        await message.reply_text("🎂 چند سالته؟\nبین ۱۰ تا ۱۰۰")
        return

    # -------------------- سن --------------------
    if state == "ask_age":
        if not text.isdigit() or not 10 <= int(text) <= 100:
            await message.reply_text("❌ سن نامعتبره. عددی بین ۱۰ تا ۱۰۰ بفرست.")
            return
    ...
    if state == "ask_province":
        if text not in PROVINCES and text != OTHER_PROVINCE:
            await message.reply_text("لطفاً از دکمه‌های استان انتخاب کن.", reply_markup=PROVINCE_MENU)
            return
        update_user(tg_id, province=text, state="idle", anon_code=anon_code())
        await message.reply_text(
        ...
        try:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("👁 نمایش پیام", callback_data=f"reveal_{mid}")
                ]])
            )
            update_user(tg_id, state="idle", anon_target=None)
            await message.reply_text("✅ پیام ناشناس ارسال شد.")
        except Exception:
            await message.reply_text("❌ ارسال پیام انجام نشد.")
        return
    ...
    if state == "support":
        try:
            await context.bot.send_message(
                ADMIN_ID,
                f"🆘 پیام پشتیبانی\n👤 کاربر: #{user.get('user_code')}\n🆔 ID: {tg_id}"
            )
            await context.bot.copy_message(ADMIN_ID, chat_id, message.message_id)
            update_user(tg_id, state="idle")
            await message.reply_text("✅ پیامت برای پشتیبانی ارسال شد.", reply_markup=MAIN_MENU)
        except Exception:
            await message.reply_text("❌ ارسال به پشتیبانی انجام نشد.")
        return

    # -------------------- شروع جستجوی چت --------------------
    if text in ("👧 چت با دختر 💕", "👦 چت با پسر 🔥"):
        if not user.get("gender"):
            await message.reply_text("اول پروفایلت رو کامل کن.")
            return

        if state == "chatting":
            await message.reply_text("⚠️ الان داخل یک چت هستی.", reply_markup=CHAT_MENU)
            return

        wanted = "female" if "دختر" in text else "male"

        # اگر قبلاً منتظر بود، اول پاکش کن
        if state == "waiting":
            update_user(tg_id, state="idle", waiting_for=None)

        match = find_match(tg_id, user["gender"], wanted)

        if not match:
            update_user(tg_id, state="waiting", waiting_for=wanted)
            await message.reply_text("⏳ در حال پیدا کردن همراه چت...\nیکم صبر کن 🔍")
            return

        partner_id = match["tg_id"]

        update_user(tg_id, state="chatting", partner_id=partner_id, waiting_for=None)
        update_user(partner_id, state="chatting", partner_id=tg_id, waiting_for=None)

        me = get_user(tg_id)
        partner = get_user(partner_id)

        await message.reply_text(
            f"✅ به «{partner.get('nickname') or 'کاربر'}» (#{partner.get('user_code')}) وصل شدی!\n\n💬 گفتگو رو شروع کن.",
            reply_markup=CHAT_MENU
        )
        await context.bot.send_message(
            partner_id,
            f"✅ به «{me.get('nickname') or 'کاربر'}» (#{me.get('user_code')}) وصل شدی!\n\n💬 گفتگو رو شروع کن.",
            reply_markup=CHAT_MENU
        )
        ...
        try:
            await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("👤 نمایش پروفایل", callback_data=f"viewprofile_{partner_id}")
                ]])
            )
            await context.bot.send_message(
                partner_id,
    ...
    if text == "👤 پروفایل من":
        ...
        count = db.execute("SELECT COUNT(*) FROM users WHERE referred_by=?", (tg_id,)).fetchone()[0]
        caption = profile(user) + f"\n🎁 دعوت موفق: {count}"

        if user.get("photo_file_id"):
            await context.bot.send_photo(chat_id, user["photo_file_id"], caption=caption)
        else:
            await message.reply_text(caption)

        update_user(tg_id, state="profile_menu")
        await message.reply_text("✏️ چی رو می‌خوای ویرایش کنی؟", reply_markup=PROFILE_MENU)
        return
    ...
    if state == "profile_menu":
        ...
        if text == "✏️ ویرایش عکس":
            ...
            await message.reply_text("عکس جدیدت رو بفرست:", reply_markup=SKIP_MENU)
            return
        if text == "✏️ ویرایش سن":
        ...
        if text == "✏️ ویرایش شهر":
            ...
            await message.reply_text("شهر جدیدت رو بنویس:", reply_markup=REMOVE_MENU)
            return
        ...
        await message.reply_text("یکی از گزینه‌ها رو انتخاب کن.", reply_markup=PROFILE_MENU)
        return
    ...
    if state == "edit_nickname":
        ...
        await message.reply_text("✅ اسم آپدیت شد.", reply_markup=MAIN_MENU)
        return

    # -------------------- ویرایش سن --------------------
    if state == "edit_age":
        if not text.isdigit() or not 10 <= int(text) <= 100:
            await message.reply_text("سن باید بین ۱۰ تا ۱۰۰ باشه.")
            return
        update_user(tg_id, age=int(text), state="idle")
        await message.reply_text("✅ سن آپدیت شد.", reply_markup=MAIN_MENU)
        return
    ...
    if state == "edit_city":
        ...
        await message.reply_text("✅ شهر آپدیت شد.", reply_markup=MAIN_MENU)
        return
    ...
    if state == "edit_photo":
        if text == "⏭ رد کردن":
            ...
            return
        if message.photo:
            update_user(tg_id, photo_file_id=message.photo[-1].file_id, state="idle")
            await message.reply_text("✅ عکس پروفایل آپدیت شد.", reply_markup=MAIN_MENU)
            return
        await message.reply_text("📷 یک عکس بفرست.", reply_markup=SKIP_MENU)
        return

    # -------------------- ویرایش استان --------------------
    if state == "edit_province":
        if text not in PROVINCES and text != OTHER_PROVINCE:
            await message.reply_text("لطفاً استان رو از دکمه‌ها انتخاب کن.", reply_markup=PROVINCE_MENU)
            return
        update_user(tg_id, province=text, state="idle")
        await message.reply_text("✅ استان آپدیت شد.", reply_markup=MAIN_MENU)
        return

    # -------------------- کنترل‌های چت (نفر بعدی / گزارش / پایان) --------------------
    if text in ("⏭ نفر بعدی", "⛔️ پایان چت", "🚩 گزارش کاربر"):
        if state != "chatting" or not user.get("partner_id"):
            await message.reply_text("❌ شما الان داخل چتی نیستید.", reply_markup=MAIN_MENU)
            return

        partner_id = user["partner_id"]
        previous_wanted = user.get("waiting_for")  # برای نفر بعدی

        # گزارش
        if text == "🚩 گزارش کاربر":
            if not blocked(tg_id, partner_id):
                db.execute(
                    "INSERT INTO blocked_pairs (reporter_id, reported_id) VALUES (?, ?)",
                    (tg_id, partner_id)
                )
                db.commit()
            reply = "🚩 کاربر گزارش شد.\nدیگه هیچ‌وقت به هم وصل نمی‌شید."

        # نفر بعدی
        elif text == "⏭ نفر بعدی":
            reply = None  # بعداً پیام جدا می‌دیم
        ...
        # قطع ارتباط
        update_user(tg_id, state="idle", partner_id=None, waiting_for=None)
        update_user(partner_id, state="idle", partner_id=None, waiting_for=None)
        ...
        # اگر «نفر بعدی» بود → خودکار دوباره جستجو کن
        if text == "⏭ نفر بعدی":
            # سعی می‌کنیم جنسیت قبلی رو حفظ کنیم
            wanted = previous_wanted
            if not wanted:
                # اگر ذخیره نشده بود، از روی جنسیت خود کاربر حدس بزن (fallback)
                wanted = "female" if user.get("gender") == "male" else "male"
            ...
            if match:
                ...
                await message.reply_text(
                    "🔁 چت قبلی تموم شد.\n"
                    "⏳ دارم دنبال نفر بعدی می‌گردم... یکم صبر کن 🔍",
                    reply_markup=CHAT_MENU
                )
            return
    ...
    # -------------------- رله پیام‌های چت --------------------
    if state == "chatting" and user.get("partner_id"):
        try:
    ...
    # -------------------- پیش‌فرض --------------------
    # اگر منتظر بود و پیام معمولی فرستاد، از حالت انتظار خارجش کن
    if state == "waiting":
        update_user(tg_id, state="idle", waiting_for=None)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("reveal_"):
        try:
            message_id = int(data.replace("reveal_", "", 1))
        except ValueError:
            await query.answer("❌ پیام نامعتبره.", show_alert=True)
            return

        cur = db.execute("SELECT * FROM anon_messages WHERE id=?", (message_id,))
        row = cur.fetchone()
        if not row:
        ...
        try:
            ...
            db.execute("UPDATE anon_messages SET revealed=1 WHERE id=?", (message_id,))
            db.commit()
            await query.answer()
        except Exception:
            await query.answer("❌ پیام دیگر در دسترس نیست.", show_alert=True)
        return

    if data.startswith("viewprofile_"):
        try:
            target_id = int(data.replace("viewprofile_", "", 1))
        except ValueError:
            await query.answer("❌ پروفایل نامعتبره.", show_alert=True)
            return

        viewer = get_user(tg_id)
        if not viewer or viewer.get("partner_id") != target_id:
            await query.answer("❌ این پروفایل دیگر در دسترس نیست.", show_alert=True)
            return
        ...
        if target.get("photo_file_id"):
            await context.bot.send_photo(chat_id=chat_id, photo=target["photo_file_id"], caption=text_profile)
        else:
            await context.bot.send_message(chat_id=chat_id, text=text_profile)
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    profiles = db.execute("SELECT COUNT(*) FROM users WHERE gender IS NOT NULL").fetchone()[0]
    ...
    referrals = db.execute("SELECT COUNT(*) FROM users WHERE referred_by IS NOT NULL").fetchone()[0]
...
def set_webhook():
    ...
    if not base_url:
        return (
            ...
            "مثال:<br>?url=https://USERNAME.pythonanywhere.com"
        ), 400

    webhook_url = base_url.rstrip("/") + f"/webhook/{TOKEN}"
    ...
    try:
        ...
        return (
            f"✅ Webhook با موفقیت تنظیم شد!<br><br>"
            f"🔗 {webhook_url}<br><br>"
            f"Telegram: {result}"
        )
    except Exception as e:
        logger.exception("Webhook setup error")
        return f"❌ خطا در تنظیم Webhook:<br>{e}", 500
